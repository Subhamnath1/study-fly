...
if __name__ == "__main__":
    print(f"{'Key #':<6} | {'Gemini 2.5':<12} | {'Gemini 3.0':<12}")
    print("-" * 40)
    sys.stdout.flush()
    for i, key in enumerate(API_KEYS):
        res25 = test_key(key, i, 'gemini-2.5-flash')
        res30 = test_key(key, i, 'gemini-3-flash-preview')
        print(f"Key {i+1:<2} | {res25:<12} | {res30:<12}")
        sys.stdout.flush()
        time.sleep(0.5)
