...
// Format duration in minutes to "1h 55m" style
...
async function sendScheduleForDay(chatId, env, dayOffset, title) {
    ...
    for (const s of entry.subjects) {
        const dur = s.duration ? ` (${formatDurationMins(s.duration)})` : '';
        msg += `🔹 <b>${htmlEscape(s.subject)}</b> @ ${s.unlockTime}${dur}\n`;
...
${subj.duration ? `🕰 <b>Duration:</b> ${formatDurationMins(subj.duration)}` : ''}
